...
const ProjectDetails = ({project, status, setStatus}) => {
    ...
    return (
        <div className={`overlay ${status ? 'block' : 'hidden'} fixed top-0 left-0 bg-black/75 w-full h-full z-50`}>
            <div className={`project px-3 pb-3 border-2 bg-gray-900 fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-11/12 h-4/5 z-20 text-white border-gray-700 overflow-y-scroll`}>
                ...
                <h3 className='text-2xl font-bold my-3 text-white relative'>{project?.name}<span className='absolute top-1/2 -translate-y-1/2 right-0 text-xs font-light bg-blue-600 px-3 py-1 rounded-2xl'>{project?.stack}</span></h3>
                ...
                <div className="flex flex-col justify-between gap-5 mt-10">
                    ...
                    <a className='py-2 w-full text-center bg-blue-600 font-medium hover:bg-blue-700 duration-300 flex items-center justify-center' href={project?.githubServer} target="_blank" rel="noopener noreferrer"><FaGithub className='mr-1' />Server</a>
                </div>
            </div>
        </div>
    );
