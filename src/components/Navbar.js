            ...
            <div className="container px-4 mx-auto md:px-8">
                <div className="flex items-center justify-between">
                    ...
                    <ul className={`menu text-white bg-gradient-to-b from-black to-gray-800 top-0 ${isOpen ? 'left-0 lg:left-2/3 lg:w-1/3' : 'left-[110%]'} absolute h-screen w-screen flex flex-col justify-center -z-10 duration-700 drop-shadow-xl lg:drop-shadow-2xl`}>
                        {
                            navLink.map(({id, link}) => (
                                <li key={id} onClick={hamburger} className='text-2xl text-center my-3 cursor-pointer hover:text-blue-600 font-medium duration-300 xl:my-4'><Link to={link} smooth duration={700} onClick={hamburger} className="block">{link}</Link></li>
                            ))
                        }
                    </ul>
